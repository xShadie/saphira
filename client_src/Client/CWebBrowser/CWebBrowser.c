...
/******************************* DisplayHTMLStr() ****************************
 * Takes a std::string containing some HTML BODY, and displays it in the specified
 * window. For example, perhaps you want to display the HTML text of...
 *
 * <P>This is a picture.<P><IMG src="mypic.jpg">
 *
 * hwnd =		Handle to the window hosting the browser object.
 * std::string =		Pointer to nul-terminated std::string containing the HTML BODY.
...
{	
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		// write() to stuff our HTML std::string into it.
		VariantInit(&myURL);
		myURL.vt = VT_BSTR;
...
long DisplayHTMLPage(HWND hwnd, LPTSTR webPageName)
{
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		// char). The std::string is not nul-terminated. The OS function SysAllocString can allocate and
		// copy a UNICODE C std::string to a BSTR. Of course, we'll need to free that BSTR after we're done
