...
bool CActorInstance::IsAttachedSpecialEffect(DWORD dwEID)
{
	std::list<TAttachingEffect>::iterator it = m_AttachingEffectList.begin();

	for (; it != m_AttachingEffectList.end(); ++it)
	{
		if (it->dwEffectIndex == dwEID)
			return true;
	}
