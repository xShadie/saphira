...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		else if (pkInstEach->IsNPC())
		{
			...
			if (aMarkPosition.m_eNameColor == CInstanceBase::NAMECOLOR_OFFLINE_SHOP ||
				aMarkPosition.m_eNameColor == CInstanceBase::NAMECOLOR_OFFLINE_SHOP_OWN ||
				aMarkPosition.m_eNameColor == CInstanceBase::NAMECOLOR_OFFLINE_SHOP_LOCKED)
