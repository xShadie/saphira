...
UINT CInstanceBase::GetNameColorIndex()
{
	...
	else if (IsNPC())
	{
		...
		IAbstractPlayer& rPlayer = IAbstractPlayer::GetSingleton();
		if (rPlayer.IsOwnOfflineShop(GetVirtualID()))
		{
			static DWORD effect = 0;
			if (!m_GraphicThingInstance.IsAttachedSpecialEffect(effect))
				effect = __AttachEffect(CInstanceBase::EFFECT_MY_OFFLINE_SHOP);

			if (rPlayer.GetOfflineShopState())
				return NAMECOLOR_OFFLINE_SHOP_LOCKED;

			return NAMECOLOR_OFFLINE_SHOP_OWN;
		}
