...
#ifdef ENABLE_MULTI_LANGUAGE
				if (pLanguageInstance)
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - pLanguageInstance->GetWidth() - 4.0f, pTextTail->y - 10.0f);
#endif	
			}
		}
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight, bool IsPC)
{
	...
	if (!pCharacterInstance->IsSA() && !pCharacterInstance->IsBuffNPC(pCharacterInstance->GetVirtualID())) 
#else
