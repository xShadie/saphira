...
void CSlotWindow::DeactivateSlot(DWORD dwIndex)
{
	...
	if (!m_pSlotActiveEffect[itemSize]) {
		__DestroySlotEnableEffect(itemSize);
	}
#endif
}
...
void CSlotWindow::ActivateSlotEffect(DWORD dwIndex, float r, float g, float b, float a)
{
	TSlot* pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
		return;
	...
	for (int i = 0; i < 3; ++i) {
		...
		for (int j = 0; j <= 12; ++j) {
			...
			pEff->AppendImage(cBuf, 0.0f, 0.0f, r, g, b, a);
		}
...
)
{
	...
	for (int i = 0; i < 13; i++) {
		char cBuf[72];
		sprintf_s(cBuf, "d:/ymir work/ui/public/slotactiveeffect/slot%d/%02d.sub", (activeSlot + 1), i);
		m_pSlotActiveEffect[activeSlot]->AppendImage(cBuf);
	}

	m_pSlotActiveEffect[activeSlot]->SetDelay(2);
