...
void CHARACTER_MANAGER::CheckEventForDrop(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM>& vec_item)
{
	...
	if (eventPtr)
	{
		...
		if (prob < success_prob)
		{
			...
			constexpr DWORD m_lbookItems[] = { 55003, 55004, 55005, 55010, 55011, 55012, 55013, 55014, 55017, 55018, 55019, 55019, 55020, 55021, 50516, 50517, 50518, 50519, 50520, 50521, 50522, 5053, 50525, 50560, 50561, 50562, 50563, 50564, 50565, 50566, 50567, 50333, 50334, 50335, 50336 };
			std::vector<LPITEM> m_cache;
			for (const auto& vItem : vec_item)
			{
	...
	if (eventPtr)
	{
		...
		{
			// If you have different book index put here!
			constexpr DWORD m_lticketItems[] = { 55003, 55004, 55005, 55010, 55011, 55012, 55013, 55014, 55017, 55018, 55019, 55019, 55020, 55021, 50516, 50517, 50518, 50519, 50520, 50521, 50522, 5053, 50525, 50560, 50561, 50562, 50563, 50564, 50565, 50566, 50567, 50333, 50334, 50335, 50336 };
			std::vector<LPITEM> m_cache;
			for (const auto& vItem : vec_item)
			{
				const DWORD itemVnum = vItem->GetVnum();
...
void CHARACTER_MANAGER::SetEventStatus(const WORD eventID, const bool eventStatus, const int endTime)
{
	...
	const std::map<BYTE, std::pair<std::string, std::string>> m_eventText = {
		{BONUS_EVENT,{"3018","3019"}},
		{DOUBLE_BOSS_LOOT_EVENT,{"3020","3021"}},
		{DOUBLE_METIN_LOOT_EVENT,{"3022","3023"}},
		{DOUBLE_MISSION_BOOK_EVENT,{"3024","3025"}},
		{DUNGEON_COOLDOWN_EVENT,{"3026","3027"}},
		{DUNGEON_TICKET_LOOT_EVENT,{"3028","3029"}},
		{MOONLIGHT_EVENT,{"3030","3031"}},
	};
