		...
		else
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					_wuint32 addVnum = dwVnum;
					_wuint32 startRefineVnum = GetWikiItemStartRefineVnum(dwVnum);
		...
		else if ( strType == "limit" )
		{
			...
			for ( int k=1; k < 256; k++ )
			{
				...
				if ( loader.GetTokenVector(buf, &pTok) )
				{
					...
					_wuint32 currVnum = dwItemVnum;
					_wuint32 startRefineVnum = GetWikiItemStartRefineVnum(dwItemVnum);
