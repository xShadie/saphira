...
bool CItemshopManager::CanBuyItem(LPCHARACTER ch, const char* hash, WORD wCount)
{
	if (!ch->CanUseItemshop())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;5000]");
		return false;
	}
	...
	{
		ch->SetItemshopCooldown(thecore_pulse() + PASSES_PER_SEC(10));
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;5001]");
		return false;
	}
	...
	if (!item)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;5002]");
		return false;
	}
	...
	if (item->IsDragonSoul())
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "[LS;445]");
...
bool CItemshopManager::BuyItem(LPCHARACTER ch, const char* hash, WORD wCount)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "[LS;5003]");
