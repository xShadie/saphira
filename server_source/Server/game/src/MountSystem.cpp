...
bool CMountActor::Mount(LPITEM mountItem)
{
	...
	DWORD myMountVnum = m_pkOwner->GetMountVnum();
#ifdef ENABLE_COSTUME_MOUNT
	DWORD dwMountSkinvnum = m_pkOwner->GetMountSkinVnum();
	...
	DWORD dwTime = mountItem->IsUnlimitedTimeUnique() == true ? 86400 : mountItem->GetSocket(0) - time(0);
