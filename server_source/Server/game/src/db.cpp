		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					else
					{
						char szQuery[1024];
#ifdef ENABLE_LANGUAGE_FLAGS
						char szLanguage[LANGUAGE_MAX_LEN * 2 + 1];
						DBManager::instance().EscapeString(szLanguage, sizeof(szLanguage), pinfo->szLanguage, strlen(pinfo->szLanguage));

						snprintf(szQuery, sizeof(szQuery), "UPDATE account SET last_play=NOW(), language='%s' WHERE id=%u", szLanguage, dwID);
#else
