		...
		if (pOld)
		{
			if (wCell < INVENTORY_MAX_NUM)
			{
				for (int i = 0; i < pOld->GetSize(); ++i)
				{
					...
					m_pointsInstant.playerSlots->wItemGrid[p] = 0;
				}
			}
			else
				m_pointsInstant.playerSlots->wItemGrid[wCell] = 0;
		}
		...
		if (pItem)
		{
			if (wCell < INVENTORY_MAX_NUM)
			{
				for (int i = 0; i < pItem->GetSize(); ++i)
				{
					...
					m_pointsInstant.playerSlots->wItemGrid[p] = wCell + 1;
				}
			}
			else
				m_pointsInstant.playerSlots->wItemGrid[wCell] = wCell + 1;
		}
...
#ifdef ENABLE_PERFORMANCE_TEST
	return m_pointsInstant.playerSlots->pItems[INVENTORY_MAX_NUM + wCell];
#else
...
bool CHARACTER::IsEmptyItemGrid(TItemPos Cell, BYTE bSize, int iExceptionCell) const
{
	...
	case INVENTORY:
	{
		...
		if (Cell.IsBeltInventoryPosition())
		{
			...
			if (m_pointsInstant.playerSlots->wItemGrid[wCell])
			{
				if (m_pointsInstant.playerSlots->wItemGrid[wCell] == iExceptionCell)
					return true;
		...
		if (m_pointsInstant.playerSlots->wItemGrid[wCell])
		{
			if (m_pointsInstant.playerSlots->wItemGrid[wCell] == iExceptionCell)
			{
				...
				do
				{
					...
					if (m_pointsInstant.playerSlots->wItemGrid[p])
						if (m_pointsInstant.playerSlots->wItemGrid[p] != iExceptionCell)
							return false;
				} while (++j < bSize);
...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	if (GetGold() < cost)
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1173]");
		return false;
	}
	...
	if (!bMoneyOnly && !RefineChance)
	{
		for (int i = 0; i < prt->material_count; ++i)
		{
			if (CountSpecifyItem(prt->materials[i].vnum, item->GetCell()) < prt->materials[i].count)
			{
				...
				ChatPacket(CHAT_TYPE_INFO, "[LS;1701]");
				return false;
			}
		}
...
int CalculateConsumeSP(LPCHARACTER lpChar)
{
	...
	if (curSP < needSP)
	{
		lpChar->ChatPacket(CHAT_TYPE_INFO, "[LS;1062]");
		return -1;
	}
							...
							case ITEM_ELK_VNUM:
								{
									...
									ITEM_MANAGER::instance().RemoveItem(item);
									ChatPacket(CHAT_TYPE_INFO, "[LS;1269;%lld]", lldGold);
					...
					case USE_TALISMAN:
						{
							...
							else if (item->GetValue(0) == MEMORY_PORTAL)
							{
								if (item->GetSocket(0) == 0)
								{
									if (GetDungeon())
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;391;[IN;%d];%s]", item->GetVnum(), "");
										return false;
									}
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							{
								ChatPacket(CHAT_TYPE_INFO, "[LS;164]");
								return false;
							}
							...
							switch (item->GetSubType())
							{
								...
								case USE_CHANGE_COSTUME_ATTR:
									{
										...
										if (item2->GetCostumeAttrCount() == 0)
										{
								...
								case USE_CHANGE_ATTRIBUTE :
									...
									if (item->GetVnum() != 71052 && item2->GetAttributeCount() == 0 || item->GetVnum() == 71052 && item2->GetRareAttrCount() == 0)
									{
										ChatPacket(CHAT_TYPE_INFO, "[LS;397]");
										return false;
									}
									...
									else
									{
										...
										{
											if ((item2->GetType() == ITEM_WEAPON) || (item2->GetType() == ITEM_ARMOR && item2->GetType() == ARMOR_BODY) || (item2->GetType() == ITEM_ARMOR && item2->GetType() == ARMOR_PENDANT))
											{
												bool bCanUse = true;
												for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
												{
								...
								case USE_ADD_ATTRIBUTE :
									...
									else if (item2->GetAttributeCount() < 4)
									{
										...
										{
											if ((item2->GetType() == ITEM_WEAPON) || (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY) || (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_PENDANT))
											{
												bool bCanUse = true;
												for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
												{
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (IS_SUMMON_ITEM(item->GetVnum()))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;98]");
			return false;
		}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;769]");
		return false;
	}
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
	...
	ChatPacket(CHAT_TYPE_INFO, "[LS;4002;[IN;%d]]", item->GetVnum());
...
{
	ChatPacket(CHAT_TYPE_INFO, "[LS;373]");
	return false;
}
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							if (wCount == 0)
							{
								if (item->GetCount() > 1)
								{
									ChatPacket(CHAT_TYPE_INFO, "[LS;24;[IN;%d];%d]", item2->GetVnum(), item->GetCount());
								}
								else
								{
				...
				if (item->GetCount() > 1)
				{
					ChatPacket(CHAT_TYPE_INFO, "[LS;24;[IN;%d];%d]", item->GetVnum(), item->GetCount());
				}
				else
				{
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
					...
					if (item2->GetVnum() == item->GetVnum())
					{
						...
						if (iCount == 0)
						{
							if (owner == this)
							{
								if (item->GetCount() > 1)
								{
									ChatPacket(CHAT_TYPE_INFO, "[LS;24;[IN;%d];%d]", item->GetVnum(), item2->GetCount());
								}
								else
								{
									ChatPacket(CHAT_TYPE_INFO, "[LS;444;[IN;%d]]", item2->GetVnum());
							...
							else
							{
								if (item->GetCount() > 1)
								{
									owner->ChatPacket(CHAT_TYPE_INFO, "[LS;4000;%s;%d;[IN;%d]]", GetName(), item->GetCount(), item2->GetVnum());
								}
								else
								{
									owner->ChatPacket(CHAT_TYPE_INFO, "[LS;4001;%s;[IN;%d]]", GetName(), item2->GetVnum());
								}
							}
							M2_DESTROY_ITEM(item);
							return true;
			...
			if (owner == this)
			{
				if (item->GetCount() > 1)
				{
					ChatPacket(CHAT_TYPE_INFO, "[LS;445;[IN;%d];%d]", item->GetVnum(), item->GetCount());
				}
				else
				{
					ChatPacket(CHAT_TYPE_INFO, "[LS;444;[IN;%d]]", item->GetVnum());
			...
			else
			{
				if (item->GetCount() > 1)
				{
					owner->ChatPacket(CHAT_TYPE_INFO, "[LS;4000;%s;%d;[IN;%d]]", GetName(), item->GetCount(), item->GetVnum());
				}
				else
				{
					owner->ChatPacket(CHAT_TYPE_INFO, "[LS;4001;%s;[IN;%d]]", GetName(), item->GetVnum());
				}
			}
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1026]");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;470]");
		return false;
	}
	...
	if (item->IsDragonSoul())
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;1404]");
			return false;
		}
...
bool CHARACTER::CanReceiveItem(LPCHARACTER from, LPITEM item) const
{
	...
	switch (GetRaceNum())
	{
		...
		case DEVILTOWER_BLACKSMITH_ARMOR_MOB:
			if (item->GetType() == ITEM_ARMOR && (item->GetSubType() == ARMOR_BODY || item->GetSubType() == ARMOR_SHIELD || item->GetSubType() == ARMOR_HEAD || item->GetSubType() == ARMOR_PENDANT) && item->GetRefinedVnum())
				return true;
			else
				return false;
			break;
...
void CHARACTER::ReceiveItem(LPCHARACTER from, LPITEM item)
{
	...
	switch (GetRaceNum())
	{
		...
		case DEVILTOWER_BLACKSMITH_ACCESSORY_MOB:
			...
			{
				from->ChatPacket(CHAT_TYPE_INFO, "[LS;991]");
			}
			break;
	...
	for (int i = 0; i < n; i++)
	{
		...
		switch (dwVnum)
		{
			case CSpecialItemGroup::GOLD:
				...
				ChatPacket(CHAT_TYPE_INFO, "[LS;1269;%d]", dwCount);
				bSuccess = true;
				break;
			case CSpecialItemGroup::EXP:
				{
					...
					ChatPacket(CHAT_TYPE_INFO, "[LS;1290;%d]", dwCount);
					bSuccess = true;
				}
				break;
			...
			default:
				{
					...
					{
						if (dwCount > 1)
							ChatPacket(CHAT_TYPE_INFO, "[LS;24;[IN;%d];%d]", item_get->GetVnum(), dwCount);
						else
							ChatPacket(CHAT_TYPE_INFO, "[LS;444;[IN;%d]]", item_get->GetVnum());
						bSuccess = true;
					}
				}
				break;
...
bool CHARACTER::CanEquipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell)
{
	...
	else if (item->GetType() == ITEM_COSTUME && item->GetSubType() == COSTUME_WEAPON)
	{
		...
		if (!pkItem)
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;1219]");
			return false;
		}
...
bool CHARACTER::CanUnequipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell, bool checkCostume)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1095]");
		return false;
	}
	...
	if (checkCostume && item->GetType() == ITEM_WEAPON && item->GetSubType() != WEAPON_ARROW && item->GetSubType() != WEAPON_QUIVER)
	{
		...
		if (pkItem)
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;1221]");
			return false;
		}
	}
...
void CHARACTER::AcceRefineCheckin(BYTE acceSlot, TItemPos currentCell)
{
	...
	if (ABSORB == GetAcceWindowType())
	{
		...
		else if (acceSlot == ACCE_SLOT_LEFT)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "[LS;1122]");
				return;
			}
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;667]");
		return;
	}
	...
	if (COMBINE == GetAcceWindowType())
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "[LS;1124]");
			return;
		}
		...
		if (pBaseItem && pMaterialItem)
		{
			...
			if (prob <= prt->prob)
			{
				...
				if (pkNewItem)
				{
					...
					if (newDrain >= 19 && chatType == 1)
					{
						ChatPacket(CHAT_TYPE_NOTICE, "[LS;1131;%s;%d;[IN;%d]]", GetName(), newDrain, pkNewItem->GetVnum());
					}
					else if (chatType == 2)
					{
						if (newDrain == 25)
						{
							ChatPacket(CHAT_TYPE_INFO, "[LS;1206]");
						}
						else
						{
							ChatPacket(CHAT_TYPE_INFO, "[LS;1204;%d]", newDrain);
						}
					}
